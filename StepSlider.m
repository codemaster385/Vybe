...
- (void)layoutLayersAnimated:(BOOL)animated
{
    ...
    _trackLayer.fillColor       = [self.sliderCircleColor CGColor];
...
{
    return self.sliderPosition + diff >= [self trackCirclePosition:trackCircle] ? [self.sliderCircleColor CGColor] : self.trackColor.CGColor;
}
